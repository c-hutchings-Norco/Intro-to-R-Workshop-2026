...
- Create a named vector M-F temps
```{r}
temps <- c(...)
temps
```
...
# If Kaggle is not loaded than use pip install kaggle in command prompt
...
apply(X=student_admission, ...) %>% #MARGIN=2 means column fun=function
...
student_admission <- student_admission %>%
  mutate(
    ...
    HS_Percentage = replace_na(HS_Percentage, ...),
    TestScore = replace_na(TestScore, ...)
  )
